...
DRS server は提示された Passport 中の各 Visa を検証し、認可を判断する。

- 署名アルゴリズムを RS256 / ES256 に限定する（`none` や HMAC を拒否し、alg confusion を防ぐ）。
- `iss` を trusted-issuer allowlist で確認する。
- 鍵解決に用いる `jku` が当該 `iss` に対して信頼済みかを、fetch 前に検証する。
- `exp`（必須）が未来であること、`iat` が妥当であることを確認する。
- `ga4gh_visa_v1.type` が `ControlledAccessGrants` であること、`value` が対象 object の所属 dataset の識別子と一致すること、`by` が許容値であることを確認する。
...
- `GET /permissions/{user}`: 利用者の grant に基づく Passport（`ga4gh_passport_v1`）を返す。
...
- 停止は、発行済み session token の失効によって配信 request 単位で反映される。
...
- grant 剥奪を即時に配信へ反映する。
- 配信を byte 単位で audit できる（配信 request ごとに認可判断と記録を行う）。
...
- at-rest 暗号化の鍵管理（鍵の置き場所と rotation の方針）。
